...
(function() {
  function DuyKhanhProtect() {
    ...
    var DuyKhanhContent = "-DuyKhanh-";
    var DuyKhanh = document.getElementById("Telegram_@cardvip2s");
    if (DuyKhanh === null) {
      alert('Phát hiện source code đã bị chỉnh sửa - error 0x000001 ')
      window.location.replace("//facebook.com/DuyKhanhRealL01");
      return;
    } else if (DuyKhanh.innerHTML !== DuyKhanhContent){
      return;
    }
  }
  DuyKhanhProtect();
...
// ABOUT ME
document.getElementById("DuyKhanhAbout_1").innerHTML = 'nguyenduykhanh.vn is a free source code sharing blog with quality and diverse content, we want to become a trusted destination for the programming community.';
// FOOTER
const footerElement = document.createElement("footer");
footerElement.classList.add("Cuoi", "Trang");
const pElement = document.createElement("p");
pElement.classList.add("DuyKhanhFtCopyright");
const spanElement = document.createElement("span");
spanElement.style.fontFamily = "Monomaniac One";
spanElement.classList.add("copyright");
const aElement = document.createElement("a");
aElement.style.textDecoration = "none";
aElement.href = "//facebook.com/DuyKhanhRealL01";
...
link.rel = "stylesheet";
link.type = "text/css";
link.href = "./DuyKhanhft-data/css/index.css";
...
.MangXaHoiFtDuyKhanh:after {
  content: "";
  clear: both;
  display: table;
}
.MangXaHoiFtDuyKhanh div:not(:last-child) {
  border-right: none; 
}
.MangXaHoiFtDuyKhanh div:hover {
...
//Random music
let index = Math.floor(Math.random() * DuyKhanh_List.length);
const audio = new Audio(DuyKhanh_List[index]);
function DuyKhanhAudio() {
  audio.play();
}
document.addEventListener("click", DuyKhanhAudio);
audio.addEventListener("ended", function() {
  index = (index + 1) % DuyKhanh_List.length;
  audio.src = DuyKhanh_List[index];
  audio.play();
});
function onCreate() {
  CheckFps();
...
function Youtube() {
  setTimeout(function() {
    window.open('https://youtube.com/@DuyKhanhRealL', '_blank')},
  100);
}
function Facebook() {
  setTimeout(function() {
    window.open('https://www.facebook.com/DuyKhanhRealL01', '_blank')},
  100);
}
function Discord() {
  setTimeout(function() {
